...
(function() {
  ...
  const siteConfigs = {
    studygo: {
      ...
      wordListSelectors: [
        // Practice/Results mode selectors (with feedback indicators)
        '.practice-list',
        '[class*="practice"]',
        '[class*="result"]',
        '[class*="exercise"]',
  ...
  // Helper function to clean text from feedback/result indicators
  ...
  function extractWordPairs(listElement, config) {
    ...
    rows.forEach((row, index) => {
      ...
      if (term && definition && isHeaderRow(row, term, definition)) {
        log(`Row ${index}: Skipping header row - term="${term}", definition="${definition}"`);
        return;
      }
